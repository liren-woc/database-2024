...
RC PhysicalPlanGenerator::create_plan(InsertLogicalOperator &insert_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  vector<vector<Value>> &value_groups = insert_oper.value_groups();
  InsertPhysicalOperator *insert_phy_oper = new InsertPhysicalOperator(table, std::move(value_groups));
...
RC PhysicalPlanGenerator::create_plan(SortLogicalOperator &sort_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  ASSERT(child_opers.size() == 1, "sort logical operator's child number should be 1");

  unique_ptr<PhysicalOperator> child_phy_oper;
  RC rc = create(*child_opers.front(), child_phy_oper);
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to create sort child physical operator. rc=%s", strrc(rc));
    return rc;
  }

  auto sort_operator = make_unique<SortPhysicalOperator>(std::move(sort_oper.expressions()),
      std::move(sort_oper.asc()));
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  unique_ptr<PhysicalOperator> child_physical_oper;
  if (!child_opers.empty()) {
    RC rc = create(*child_opers.front(), child_physical_oper);
    if (OB_FAIL(rc)) {
  ...
  auto *update_phy_oper = new UpdatePhysicalOperator(update_oper.table(), update_oper.field_metas(), update_oper.values());
  if (child_physical_oper) {
